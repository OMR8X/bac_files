...
class _DropdownButtonWidgetState extends State<DropdownButtonWidget> {
        ...
        child: ClipRRect(
          ...
          child: Material(
            ...
            child: DropdownButtonFormField(
              ...
              onSaved: onSave,
              hint: Text(
                widget.title == null ? selectedValue : "${widget.title} : $selectedValue",
  ...
  List<DropdownMenuItem<String>>? itemsBuilder(ThemeData themeData) {
    List<DropdownMenuItem<String>>? list = widget.list
        .map((e) => DropdownMenuItem(
              value: e,
              child: Text(widget.title == null ? e : "${widget.title} : $e", style: themeData.textTheme.labelMedium),
            ))
        .toList();
    return list;
  }
  ...
  return InputDecoration(
    ...
    errorBorder: const OutlineInputBorder(
      ...
      borderSide: BorderSide(
        width: 3,
        color: Color(0xffD93D2D),
      ),
    ),
    border: OutlineInputBorder(
      borderRadius: const BorderRadius.all(Radius.circular(0.0)),
      borderSide: BorderSide(width: 1, color: themeData.primaryColorDark),
    ),
